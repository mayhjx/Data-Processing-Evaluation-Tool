...
class UAld(Evaluation):
    ...
    def AB_data_format(self):
        ...
        dic[self.UAld] = list()
        dic[self.UAld].append(head)
        dic[self.UAld].extend([d for d in data[1:]])
    ...
    def output(self):
        ...
        output["ALD_accuracy"] = self.get_accuracy(dic[self.UAld])
        output["ALD_r"] = self.get_r(dic[self.UAld])
        output["ALD_LLMI"] = self.get_LLMI(dic[self.UAld])
        output["ALD_sn"] = self.get_sn(dic[self.UAld])
        output["ALD_isarea"] =self.get_isarea(dic[self.UAld])
        output["ALD_rt"] = self.get_rt(dic[self.UAld])
        output["ALD_isrt"] = self.get_isrt(dic[self.UAld])
        output["ALD_qc"] = self.get_qc(dic[self.UAld],  self.instrument_prefix + self.instrument_num, self.UAld)

        output["total_ALD_score"] = 0

        for k, v in output.items():
            if k.startswith("ALD") and v['score']:
